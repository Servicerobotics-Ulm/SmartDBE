...
void JoystickServiceIn_2UpcallManager::notify_upcalls(const CommBasicObjects::CommJoystick &input)
{
	for(auto it=upcalls.begin(); it!=upcalls.end(); it++) {
		(*it)->on_JoystickServiceIn_2(input);
	}
}

void JoystickServiceIn_2UpcallManager::attach(JoystickServiceIn_2UpcallInterface *upcall)
{
	upcalls.push_back(upcall);
}
void JoystickServiceIn_2UpcallManager::detach(JoystickServiceIn_2UpcallInterface *upcall)
{
