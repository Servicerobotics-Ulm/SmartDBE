...
start)
	...
	xterm -fg red -title "127.0.0.1" -e 'echo;echo "Starting local session ..."; echo;echo;cd /tmp/SmartINSystemMultiMoveRobotino.deployment; pwd; bash --login -i -e start-LocalhostTarget.sh start' &
	echo $! >> $PID_FILE
...
stop)
	...
	xterm -title "stopping 127.0.0.1" -e 'echo;echo "Starting local session ..."; echo;echo;cd /tmp/SmartINSystemMultiMoveRobotino.deployment; pwd; bash --login -i -e start-LocalhostTarget.sh stop'
	echo $! >> $PID_FILE
