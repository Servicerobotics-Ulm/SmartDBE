...
DEPLOYMENT_DIRECTORY=SmartINSystemMultiMoveRobotino.deployment/
...
###############################
echo "Sourcing pre-deployment script for Enforcer... (errors might be ignored)"
DEPLOY_LIBRARIES=""
DEPLOY_COMPONENT_FILES=""
source src/predeploy-Enforcer.sh 2>&1
...
DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_EnforcerCat1/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1="$DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1 $REFERENCED_PROJECT_EnforcerCat1/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1="$DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1 $I"
	fi
done
...
for entry in "$REFERENCED_PROJECT_EnforcerCat1"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_EnforcerCat1="$DEPLOY_COMPONENT_TCL_MODEL_FILES_EnforcerCat1 $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_EnforcerCat1 "
#########################
...
DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_SmartINJoystickServer/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer="$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer $REFERENCED_PROJECT_SmartINJoystickServer/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer="$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer $I"
	fi
done
...
## BEHAVIOR FILES
shopt -u | grep -q nullglob && changed=true && shopt -s nullglob
for entry in "$REFERENCED_PROJECT_SmartINJoystickServer"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickServer="$DEPLOY_COMPONENT_TCL_MODEL_FILES_SmartINJoystickServer $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickServer "
#########################
...
DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_SmartINJoystickManager/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager="$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager $REFERENCED_PROJECT_SmartINJoystickManager/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager="$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager $I"
	fi
done
...
## BEHAVIOR FILES
shopt -u | grep -q nullglob && changed=true && shopt -s nullglob
for entry in "$REFERENCED_PROJECT_SmartINJoystickManager"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickManager="$DEPLOY_COMPONENT_TCL_MODEL_FILES_SmartINJoystickManager $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickManager "
#########################
...
DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_SmartINJoystickNavigation/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation="$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation $REFERENCED_PROJECT_SmartINJoystickNavigation/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation="$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation $I"
	fi
done
...
## BEHAVIOR FILES
shopt -u | grep -q nullglob && changed=true && shopt -s nullglob
for entry in "$REFERENCED_PROJECT_SmartINJoystickNavigation"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickNavigation="$DEPLOY_COMPONENT_TCL_MODEL_FILES_SmartINJoystickNavigation $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickNavigation "
#########################
...
src/Enforcer_data
src/startstop-hooks-Enforcer.sh
$SMART_ROOT_ACE/lib/libSmartInstitutionsServiceRepository*
...
$SMART_ROOT_ACE/bin/EnforcerCat1
src-gen/combined-ini-files/Enforcer.ini
$SMART_ROOT_ACE/bin/SmartINJoystickServer
...
$SMART_ROOT_ACE/bin/SmartINJoystickManager
src-gen/combined-ini-files/SJMan.ini
$SMART_ROOT_ACE/bin/SmartINJoystickNavigation
...
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1 $TMPDIR/Enforcer_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_FILES_PATHS_EnforcerCat1 $TMPDIR/Enforcer_data/ 2>&1
fi

if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_EnforcerCat1" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_EnforcerCat1 $TMPDIR/behaviorFiles/ 2>&1
fi

cp -v $REFERENCED_PROJECT_EnforcerCat1/smartsoft/src/startstop-hooks.sh $TMPDIR/startstop-hooks-component-EnforcerCat1.sh 2>/dev/null
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer $TMPDIR/RobotOperator_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer $TMPDIR/RobotOperator_data/ 2>&1
fi

if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickServer" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickServer $TMPDIR/behaviorFiles/ 2>&1
fi

cp -v $REFERENCED_PROJECT_SmartINJoystickServer/smartsoft/src/startstop-hooks.sh $TMPDIR/startstop-hooks-component-SmartINJoystickServer.sh 2>/dev/null
...
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager $TMPDIR/SJMan_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickManager $TMPDIR/SJMan_data/ 2>&1
fi

if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickManager" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickManager $TMPDIR/behaviorFiles/ 2>&1
fi

cp -v $REFERENCED_PROJECT_SmartINJoystickManager/smartsoft/src/startstop-hooks.sh $TMPDIR/startstop-hooks-component-SmartINJoystickManager.sh 2>/dev/null
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation $TMPDIR/SJNav_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickNavigation $TMPDIR/SJNav_data/ 2>&1
fi

if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickNavigation" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartINJoystickNavigation $TMPDIR/behaviorFiles/ 2>&1
fi

cp -v $REFERENCED_PROJECT_SmartINJoystickNavigation/smartsoft/src/startstop-hooks.sh $TMPDIR/startstop-hooks-component-SmartINJoystickNavigation.sh 2>/dev/null
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer $TMPDIR/SafetyOfficer_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_FILES_PATHS_SmartINJoystickServer $TMPDIR/SafetyOfficer_data/ 2>&1
fi
