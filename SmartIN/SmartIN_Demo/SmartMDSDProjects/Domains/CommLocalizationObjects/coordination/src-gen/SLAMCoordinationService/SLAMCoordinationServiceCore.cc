...
#include <smartNumericCorrelationId.h>
 ...
 void SLAMCoordinationServiceCore::addNewModuleInstance(const std::string& name){
 	std::cout<<"addNewModuleInstance name:"<<name<<std::endl;
	SLAMCoordinationService mi;
	 ciInstanceMap.insert ( std::pair<std::string,SLAMCoordinationService>(name,mi));
 }
 
 int SLAMCoordinationServiceCore::initCiInstance(SmartACE::SmartComponent * component, const std::string& ciInstanceName, const std::map< std::string, CiConnection, ciLessLibC > &ciConnectionsMap){
 	std::map<std::string, SLAMCoordinationService, ciLessLibC>::iterator iter = ciInstanceMap.find(ciInstanceName);
	...
	//component name: SLAMCoordinationService
 ...
 int SLAMCoordinationServiceCore::finiCiInstance(const std::string& ciInstanceName){
 	std::map<std::string, SLAMCoordinationService>::iterator iter = ciInstanceMap.find(ciInstanceName);
	if(iter != ciInstanceMap.end()){
		return 0;
	} else {
		std::cout<<"ERROR WAS NOT ABLE TO FIND CI INSTANCE IN MAP --> this should not have happend!"<<std::endl;
...
std::string SLAMCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& inString){
	std::map<std::string, SLAMCoordinationService>::const_iterator iter = ciInstanceMap.find(ciInstanceName);
	...
	if(iter != ciInstanceMap.end()){
		...
		//std::cout<<"switchSLAMCoordinationService - compInstName: "<<componentInstanceName<<" inString: "<<inString<<" service: "<<service<<std::endl;
