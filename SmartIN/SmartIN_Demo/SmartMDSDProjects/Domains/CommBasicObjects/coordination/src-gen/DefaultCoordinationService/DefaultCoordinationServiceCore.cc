...
#include <smartNumericCorrelationId.h>
 ...
 void DefaultCoordinationServiceCore::addNewModuleInstance(const std::string& name){
	...
	DefaultCoordinationService mi;
	 ciInstanceMap.insert ( std::pair<std::string,DefaultCoordinationService>(name,mi));
 }
 
 int DefaultCoordinationServiceCore::initCiInstance(SmartACE::SmartComponent * component, const std::string& ciInstanceName, const std::map< std::string, CiConnection, ciLessLibC > &ciConnectionsMap){
 	std::map<std::string, DefaultCoordinationService, ciLessLibC>::iterator iter = ciInstanceMap.find(ciInstanceName);
	...
	//component name: DefaultCoordinationService
 ...
 int DefaultCoordinationServiceCore::finiCiInstance(const std::string& ciInstanceName){
 	std::map<std::string, DefaultCoordinationService>::iterator iter = ciInstanceMap.find(ciInstanceName);
	if(iter != ciInstanceMap.end()){
		return 0;
	} else {
		std::cout<<"ERROR WAS NOT ABLE TO FIND CI INSTANCE IN MAP --> this should not have happend!"<<std::endl;
...
std::string DefaultCoordinationServiceCore::switchCi(const std::string& ciInstanceName, const std::string& componentName, const std::string& componentInstanceName, const std::string& service, const std::string& inString){
	std::map<std::string, DefaultCoordinationService>::const_iterator iter = ciInstanceMap.find(ciInstanceName);
